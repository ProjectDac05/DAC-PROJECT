...
BEGIN
    ...
    IF NEW.status != OLD.status THEN
        -- When a booking is confirmed (after payment), decrease available seats and mark seats as booked
        IF NEW.status = 'confirmed' THEN
            -- Update available seats in event
            UPDATE events e
            SET e.available_seats = e.available_seats - seat_count
            WHERE e.event_id = NEW.event_id
            AND e.available_seats >= seat_count;
        ...
        -- When a booking is cancelled, clean up everything
        ELSEIF NEW.status = 'cancelled' THEN
            -- If it was confirmed before, increase available seats
